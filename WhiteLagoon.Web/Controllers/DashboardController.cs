...
namespace WhiteLagoon.Web.Controllers
{
    public class DashboardController(IUnitOfWork unitOfWork) : Controller
    {
        ...
        {
            var totalBookings = unitOfWork.BookingRepository.GetAll(u => u.Status != SD.Status_Pending
            || u.Status == SD.Status_Cancelled);
        ...
        {
            var totalUsers = unitOfWork.ApplicationUserRepository.GetAll();
        ...
        {
            var totalBookings = unitOfWork.BookingRepository.GetAll(u => u.BookingDate >= DateTime.Now.AddDays(-30) &&
            (u.Status != SD.Status_Pending || u.Status == SD.Status_Cancelled));
        ...
        {
            var bookingData = unitOfWork.BookingRepository.GetAll(u => u.BookingDate >= DateTime.Now.AddMonths(-1) &&
            ...
            var customerData = unitOfWork.ApplicationUserRepository.GetAll(u => u.CreatedAt >= DateTime.Now.AddMonths(-1) &&
        ...
        private static RadialBarChartVM GetRadialCartDataModel(int totalCount, double currentMonthCount, double previousMonthCount)
        {
